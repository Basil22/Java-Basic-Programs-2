...
class Employee implements Comparable<Employee>{ //implement only for comparable, then the compareTo method gets overridden.
	private int id;
	private String name;
	private int salary;
...
//		};
		...
		Comparator<Employee> com = (Employee i, Employee j) -> i.getId() > j.getId() ? 1:-1; //lambda expression of the above
		
		Comparator<Employee> compName = new Comparator<Employee>() { //Comparator method

			public int compare(Employee i, Employee j) {
				if(i.getName().charAt(0) > j.getName().charAt(0)) return 1;
				else return -1;
			}
		};
